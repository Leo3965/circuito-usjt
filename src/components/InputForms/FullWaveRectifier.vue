...
import FullWave from "@/objects/FullWave";

export default defineComponent({
  name: "FullWaveRectifier",
  emits: ["FullWaveResult", "clear"],
  data() {
    return {
      FullWaveResult: {} as FullWave,
    }
  },
  methods: {
    handleCalc() {
      ...
      this.FullWaveResult = new FullWave(
          Number (rectifierInputs[0].value),
          String(rectifierInputs[1].value),
      ...
      this.$emit('FullWaveResult', result);
    },
    handleClear() {
      const rectifierInputs = this.getInputs();
      this.removeWarning(rectifierInputs);
